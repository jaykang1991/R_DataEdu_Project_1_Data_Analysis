...
rank_crop <- pa_crop %>% 
  group_by(품목) %>% 
  summarise_at(vars(생산비), list(평균_생산비=mean))
...
# 평균생산비 * 평균재배면적(ha) * 10 
for (j in 1:(ncol(area_crop))){rank_crop[rank_crop$품목 == names(area_crop[j]), '평균_생산비'] <- 
  rank_crop[rank_crop$품목 == names(area_crop[j]),'평균_생산비']*
  10*
  area_crop[,names(area_crop[j])]}

# 시각화 위해 10,000으로 나누기
# rank_crop$평균_생산비 <- rank_crop$평균_생산비/10000
rank_crop$평균_생산비 <- rank_crop$평균_생산비/10

rank_crop[rank_crop$품목 =='인삼(4년근)','평균_생산비'] <- rank_crop[rank_crop$품목 =='인삼(4년근)','평균_생산비']/4

# 평균 소득 히스토그램 비교
ggplot(data=rank_crop, aes_string(x='품목', y='평균_생산비'))+
  geom_bar(stat="identity", position = position_dodge(width=0.5))+
  ylab('평균 생산비 (만원)') +
  ggtitle('농작물 평균 연 생산비')
